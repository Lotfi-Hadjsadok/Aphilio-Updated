...
export function GeneratedAdCard({
  ...
  if (hasImage) {
    return (
      <Card className="h-fit w-full min-w-0 overflow-hidden rounded-2xl border-border/50 bg-card/90 p-0 shadow-md ring-1 ring-border/30">
        ...
        <div className="flex items-center justify-between gap-2 border-t border-border/30 px-4 py-2.5 sm:px-5">
          <a
  ...
  return (
    <Card className="relative h-fit w-full min-w-0 overflow-hidden rounded-2xl border-border/50 bg-card/90 shadow-md ring-1 ring-border/30">
      ...
      <CardContent className="flex flex-col gap-3.5 p-4 sm:p-5">
        <form action={imageFormAction} className="flex flex-col gap-3.5">
        ...
        {imageState.status === "error" ? <ErrorBanner message={imageState.message} /> : null}
      </CardContent>
    </Card>
  );
}
